...
iCOLS = {
    True: (iBIBINFO, iSCROLLINFO, iTRANS, iANALYSIS, iNUM,),
    False: (iSCROLLNAME, iSCROLLREF, iTRANS, iANALYSIS,),
}
...
CHARS = reduce(set.union, (set(x) for x in CHARS), set(),)
...
intFeatures = {BIBLICAL, SRCLN, INTERLINEAR} | FLAGS_NAME | BRACKETS_NAME

featureMeta = {
    "after": {"description": "space behind the word, if any", "values": " (space)"},
    ALTERNATIVE: {"description": "alternative reading", "values": "1"},
    BIBLICAL: {
    ...
    BOOK: {"description": "acronym of the book in which the word occurs"},
    CHAPTER: {"description": "label of the chapter in which the word occurs"},
    CORRECTION: {
        "description": "correction made by an ancient or modern editor",
        "values": "1 = modern, 2 = ancient, 3 = ancient supralinear",
    },
    FRAGMENT: {"description": "label of a fragment of a scroll"},
    "fullo": {
        "description": (
    ...
    "glypho": {"description": "representation (original source) of a word or sign"},
    "glyphe": {
        "description": "representation (ETCBC transliteration) of a word or sign",
    },
    "glyph": {"description": "representation (Unicode) of a word or sign"},
    HALFVERSE: {"description": "label of the half-verse in which the word occurs"},
    INTERLINEAR: {
        "description": (
    ...
    "lexo": {"description": "representation (original source) of a lexeme"},
    "lexe": {"description": "representation (ETCBC transliteration) of a lexeme"},
    "lex": {"description": "representation (Unicode) of a lexeme"},
    "glexo": {
        "description": (
    ...
    LINE: {"description": "label of a line of a fragment of a scroll"},
    MERR: {"description": "errors in parsing the morphology tag"},
    "morpho": {"description": "morphological tag (by Abegg)"},
    "occ": {"description": "edge feature from a lexeme to its occurrences"},
    "punco": {"description": "trailing punctuation (original source) of a word"},
    "punce": {"description": "trailing punctuation (ETCBC transliteration) of a word"},
    "punc": {"description": "trailing punctuation (Unicode) of a word"},
    REMOVED: {
        "description": "removed by an ancient or modern editor",
        "values": "1 = modern, 2 = ancient",
    },
    RECONSTRUCTION: {"description": "reconstructed by a modern editor", "values": "1"},
    SCRIPT: {
        "description": "script in which the word or sign is written if it is not Hebrew",
        "values": " ".join(SCRIPT_VALS),
    },
    SCROLL: {"description": "acronym of a scroll"},
    SRCLN: {"description": "the line number of the word in the source data file"},
    "type": {"description": "type of sign or cluster"},
    UNCERTAIN: {
        "description": "uncertain material in various degrees: higher degree is less certain",
        "values": "1 2 3 4",
    },
    VACAT: {"description": "empty, unwritten space", "values": "1"},
    VERSE: {"description": "label of the verse in which the word occurs"},
}
...
stMap = collections.OrderedDict((("", "ALL"), (1, "OK"), (0, "--"), (-1, "XX"),))
...
def showDiag():
    ...
    for (kind, reps) in sorted(diags.items()):
        ...
        for (rep, occs) in sorted(reps.items()):
            ...
            nR = {s: sum(1 for oc in occs if occs[oc] == s) for s in ("c", *stMap)}
            nR[""] = nOccs
            ...
            for ((biblical, ln), st) in sorted(
                occs.items(), key=lambda x: (-2 if x[1] == "c" else -x[1], x[0]),
            ):
        ...
        with open(f"{SOURCE_DIR}/dss_{src}.txt") as fh:
            for line in fh:
                ...
                iData = collections.defaultdict(
                    lambda: "", ((f, c) for (f, c) in zip(nCols, fields)),
                )
    ...
    for fields in data:
        ...
        if key in linesBoth:
            if scrollx != prevScrollx or fragment != prevFragment or line != prevLine:
                diag(
                    "LINE (biblical and nonbiblical)",
                    f"{scrollx:<15} {fragment:>10}:{line:>5}",
                    1,
                )
...
def tweakBiblicalRest():
    ...
    scrollsRep = (
        f"{nScrollsx} " + "names"
        if nScrollsx == nScrollso
        else f"({nScrollso} originally) names"
    )
    ...
    def showChars():
        ...
        for (name, freqs) in charsLetter.items():
            for (c, freq) in freqs.items():
                kind = "both" if c in lexes and c in transes else "only"
    ...
    for fields in data:
        ...
        if script:
            diag(
                "SCRIPT",
                script,
                "c" if script == PH_VAL else 1 if script in SCRIPT_VALS else -1,
            )
        ...
        if not isAmbi:
            kind = (
                "trans:yes lex:no"
                if isNumTrans and not isNumLex
                else "trans:no lex:yes"
                if not isNumTrans and isNumLex
                else None
            )
            if kind is not None:
                diag(
                    "NUMERAL inconsistent LEX",
                    f'{kind} trans="{wordPure}" lex="{glexo}"',
                    -1,
                )
        ...
        for (name, legal, text) in (
            ...
            for c in text:
                charsFound[c] += 1
                if c in legal or (c in digital and name == LEX and isNumLex):
                    charsMapped.add(c)
                else:
...
def parseMorph():
    ...
    def showMorpho():
        ...
        for (morph, analysis) in sorted(morphParsed.items()):
            ...
            analysisRep += " ".join(
                f"{k}={v}" for (k, v) in analysis.items() if k != MERR
            )
...
def director(cv):
    ...
    def asUni(text, asNum=False, asForeign=False):
        ...
        try:
            result = (
                "".join(
                    FOREIGNS_UNI[c] if c in FOREIGNS_UNI else CHARS_UNI[c] for c in text
                )
                if asForeign
                else "".join(
                    NUMERALS_UNI[c] if c in NUMERALS_UNI else CHARS_UNI[c] for c in text
                )
                if asNum
                else "".join(CHARS_UNI[c] for c in text)
            )
        except KeyError:
    ...
    def asRep(text, asNum=False, asForeign=False):
        ...
        try:
            result = (
                "".join(
                    FOREIGNS_REP[c] if c in FOREIGNS_REP else CHARS_UNI[c] for c in text
                )
                if asForeign
                else "".join(
                    NUMERALS_REP[c] if c in NUMERALS_REP else CHARS_REP[c] for c in text
                )
                if asNum
                else "".join(CHARS_REP[c] for c in text)
            )
        except KeyError:
    ...
    for fields in data:
        ...
        if changeLine or changeFragment or changeScroll:
            ...
            if biblical:
                biblicalLine = (
                    2 if (thisScroll, thisFragment, thisLine) in linesBoth else 1
                )
                if biblicalLine == 2:
                    diag(
                        "(NON)-BIBLICAL",
                        f"{thisScroll} {thisFragment}:{thisLine} line",
                        1,
                    )
        ...
        if punco or glypho or lexo:
            ...
            if biblical:
                if biblicalLine == 2:
                    diag(
                        "(NON)-BIBLICAL",
                        f"{thisScroll} {thisFragment}:{thisLine} word {fullo}",
                        1,
                    )
                if biblicalLine:
            ...
            if punco:
                cv.feature(
                    curWord, punco=punco, punc=asUni(punco), punce=asRep(punco),
                )
            if not glypho:
                typ = EMPTY
                c = ""
                ...
                if isOpen:
                    ...
                    if biblical:
                        if biblicalLine == 2:
                            diag(
                                "(NON)-BIBLICAL",
                                f"{thisScroll} {thisFragment}:{thisLine} cluster {name}",
                                1,
                            )
                        if biblicalLine:
